...
`.env` 文件采用 `KEY=VALUE` 的逐行解析格式（类 INI），支持注释：
...
| 规则   | 说明                                                                                                       |
| ------ | ---------------------------------------------------------------------------------------------------------- |
| 注释   | 行首以 `#` 或 `;` 开头即为注释行；未加引号的值中，遇到 ` #`（空格+#）会截断行内注释                        |
| 键值对 | `KEY=VALUE`，兼容 bash 风格的 `export KEY=VALUE` 前缀                                                      |
| 引号   | 值支持引号包裹：单引号内容原样保留（不处理转义）；双引号支持 `\n` `\r` `\t` `\"` `\\` 转义；引号均会被去除 |
| 分节   | `.env` 文件中不支持 `[SECTION]` 分节（分节行会被忽略），需直接使用完整键名，如 `DATABASE_HOST=...`         |
| 空格   | 键名和值的首尾空格会被自动去除                                                                             |

### 分节键名（编程式）

`.env` 文件本身不支持分节语法，但 `Env::set()` 方法支持嵌套数组，键名会自动转换为 **大写\_连接** 格式：

```php
// 通过 set() 传入嵌套数组，等价于定义 DATABASE_HOST / DATABASE_PORT
Env::set([
    'DATABASE' => [
        'HOST' => 'localhost',
        'PORT' => 3306,
