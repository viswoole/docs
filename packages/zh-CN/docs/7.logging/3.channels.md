...
自定义日志驱动推荐继承 `Viswoole\Log\Drive` 抽象基类，`Drive` 已内置各级别快捷方法、
协程缓存（`record`）与直接写入（`write`）逻辑，只需实现 `save()` 方法定义持久化策略：
...
class DatabaseLogDrive extends Drive
{
...
> 若需完全自行实现，则须实现 `DriveInterface` 接口的全部方法（各级别快捷方法、
> `mixed`、`record`、`write`、`save`、`getRecord`、`clearRecord`），建议直接继承 `Drive` 基类。

### 带构造参数的自定义驱动示例

驱动基类同样支持通过构造函数接收 `options` 配置参数，便于接入外部服务（如 Elasticsearch）：
