...
public static function image(
    ...
    // 文件已验证通过，直接处理...
    return self::saveImage($file, $response);
}
```
...
public static function document(
    ...
    return self::saveDocument($file, $response);
}
```
...
public static function batchImages(
    ...
    foreach ($files as $index => $file) {
        $path = self::saveImageFile($file, "image_{$index}");
        $savedPaths[] = $path;
    }
...
**注意：** 多个类型使用 `|` 分隔（如 `image/png|image/jpeg`）；`*` 表示不限制类型（跳过 MIME 校验）。框架**不支持** `image/*` 这类前缀通配符，如需匹配一类文件，请逐一列出具体的 MIME 类型。
...
public static function batchUpload(
    #[FileRule('image/png|image/jpeg|image/gif|image/webp')]  // 验证所有文件都是图片
    #[InjectFile]
    array $files,           // 自动识别为数组
    Response $response
): ResponseInterface {
    ...
    foreach ($files as $index => $file) {
        try {
            ...
            if ($file->getError() !== UPLOAD_ERR_OK) {
                throw new \\RuntimeException(self::getUploadErrorMessage($file->getError()));
            }
...
class FileController
{
    ...
    public static function batchUpload(
        #[FileRule(
            fileMime: 'image/png|image/jpeg|image/gif|image/webp|application/pdf|application/msword|application/vnd.ms-excel',
