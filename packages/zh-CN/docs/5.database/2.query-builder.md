...
// 也可以传入 false 禁用空结果：此时无记录会抛出 DataNotFoundException 异常
...
当查询构造器无法表达复杂 SQL 时，可直接使用 `Db::query()` / `Db::execute()` 执行原生 SQL，并支持绑定参数防注入。

### query — 原生查询（SELECT）

`query()` 签名：`query(string|Raw $sql, array $bindings = [], bool $master = false)`，仅执行查询语句（SELECT 等），返回关联数组结果集。`$master` 为 `true` 时强制从主库读取：
...
// 带绑定参数（推荐，防止 SQL 注入）
$orders = Db::query(
    'SELECT * FROM orders WHERE user_id = ? AND amount > ? ORDER BY create_time DESC',
    [$userId, 100]
);

// 强制从主库读取（读写分离场景下）
$latest = Db::query('SELECT * FROM users WHERE id = ?', [$id], true);
```

> **注意**：`Db::query()` 只接受查询语句（SELECT 等），执行写入类语句会抛出异常。

### execute — 原生写入（INSERT/UPDATE/DELETE）

`execute()` 签名：`execute(string|Raw $sql, array $bindings = [], false|string $getId = false)`，执行写入语句，返回受影响行数；传入 `$getId`（字段名）时返回该字段的自增 ID：

```php
// 受影响行数
$affected = Db::execute('UPDATE users SET status = ? WHERE id = ?', [0, 1]);

// 写入并获取自增 ID
$newId = Db::execute(
    'INSERT INTO users (name, email) VALUES (?, ?)',
    ['张三', 'zhangsan@example.com'],
    'id'   // 传入字段名，返回该字段的自增 ID
);
```

### raw — 原生表达式

`Db::raw()` 创建原生 SQL 表达式，避免被作为字符串转义（详见上文"原始表达式"）：

```php
// 配合原生查询使用
Db::query(Db::raw('SELECT * FROM users WHERE create_time > NOW()'));
```
