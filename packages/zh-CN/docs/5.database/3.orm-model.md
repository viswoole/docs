...
事务支持多层嵌套，内层基于数据库 `SAVEPOINT` 实现独立回滚，模型操作同样适用：

```php
Db::startTransaction(function () {          // 外层事务
    $user = UsersModel::create(['name' => '张三']);
    try {
        Db::startTransaction(function () use ($user) {  // 内层事务（SAVEPOINT）
            ProfileModel::create(['user_id' => $user->id, 'bio' => '新人']);
            throw new \\RuntimeException('资料写入失败');
        });
    } catch (\\RuntimeException) {
        // 内层已独立回滚：ProfileModel 记录被丢弃，外层的用户记录不受影响
    }
    // 外层继续提交：仅用户记录落库
});
```

嵌套规则与注意事项见[数据库概览 — 嵌套事务](index.md#嵌套事务)。
