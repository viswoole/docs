...
当同一个中间件类需要在不同的注册位置使用不同配置时，可以使用 `[类名, 构造参数数组]` 格式。参数按名称或位置与容器自动注入合并（**参数仅用于构造函数**，未传入的依赖仍由容器解析）：

```php
use Viswoole\\Core\\Facade\\Middleware;

// 全局注册时传参
Middleware::register([IpWhitelistMiddleware::class, ['allowedIps' => ['192.168.1.1', '10.0.0.1']]]);

// 路由级注册时传参：同一中间件类，不同路由使用不同配置
Router::get('/api/admin', [AdminController::class, 'index'])
    ->setMiddlewares([[RateLimitMiddleware::class, ['maxAttempts' => 100]]]);

Router::get('/api/public', [PublicController::class, 'index'])
    ->setMiddlewares([[RateLimitMiddleware::class, ['maxAttempts' => 10]]]);
```

注解路由同样支持该格式：

```php
#[Controller(prefix: '/api', middlewares: [
    [RateLimitMiddleware::class, ['maxAttempts' => 60]],
])]
class UserController
{
    // ...
...
| 格式 | 示例 | 说明 |
|---|---|---|
| 类名 | `AuthMiddleware::class` | 构造参数由容器自动解析 |
| `[类名, 构造参数数组]` | `[RateLimitMiddleware::class, ['limit' => 10]]` | 参数按名称或位置与容器注入合并 |
| 闭包 | `function (Closure $handler) { ... }` | 支持依赖注入 |
| 可调用数组 | `[ClassName::class, 'method']` | 方法内自行调用 `$handler()` |
| 全局函数名 | `'some_function'` | — |

> 说明：`[类名, 构造参数数组]` 与可调用数组 `[类名, '方法名']` 通过第二元素的类型（数组 / 字符串）区分，二者不会冲突。带参中间件每次请求都会重新实例化，参数不会跨请求共享。

### 参数合并规则与边界

- 参数按名称或位置与容器自动注入合并，未传入的依赖仍由容器解析；
- 若中间件类定义了 `public static factory()` 方法，参数将注入 `factory()`（与容器 `make()` 语义一致），否则注入 `__construct`；
- `params` 中与构造函数签名不匹配的多余键会被静默忽略（与容器行为一致）；
- 若中间件类没有构造函数，`params` 会被忽略，中间件正常执行；
- **对象实例不支持传参**：`[对象实例, [参数]]` 会抛出 `InvalidArgumentException`，对象请使用 `[实例, '方法名']` 可调用格式；
- 注解路由中的 `middlewares` 受 PHP Attribute 常量表达式限制，仅支持类名与 `[类名, 参数数组]` 格式（参数须为标量或常量数组），闭包仅适用于 `Middleware::register()` 和 `setMiddlewares()`。
