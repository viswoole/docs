...
在 Swoole 协程环境中，容器使用 **Context** 按顶级协程 ID 隔离单例。这意味着每个**顶级协程**（请求链路根协程）拥有独立的服务实例，同一链路内的子协程共享实例，避免并发状态污染。
...
容器默认会对已解析的类缓存单例实例，多次 `make` 返回同一实例（协程环境下按顶级协程 ID 隔离）。只有当类显式定义了 `ALLOW_NEW_INSTANCE = true` 常量时，每次 `make` 才会创建新实例：

```php
// 默认缓存单例：多次 make 返回同一实例
...
// 如果需要每次 make 都创建新实例，在类中定义常量：
class FreshInstance
{
    public const ALLOW_NEW_INSTANCE = true;
    // ...
}
```
