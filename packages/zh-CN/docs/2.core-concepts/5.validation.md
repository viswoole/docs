...
| 扩展内置类型 | `iterable`, `object`, `mixed`, `callable`, `null`, `true`, `false` | 完整 PHP 内置类型支持 |
| 类型别名    | `boolean`, `integer`, `double`, `Closure` | 等同于对应的内置类型         |
| 联合类型    | `int\|string`                             | 参数可为多种类型之一         |
| 交集类型    | `A&B`                                     | 参数必须同时满足多个类型约束 |
| 枚举        | `StatusEnum`                              | 值必须是枚举的成员之一，支持枚举实例、名称字符串和数字索引三种输入 |
| 类/接口实例 | `RequestInterface`                        | 值必须是指定类或接口的实例，否则尝试通过容器创建 |
...
校验失败抛出的 `ValidateException` 会自动携带参数名上下文，便于定位到具体参数：

- **内置规则**：默认文案内置 `{:name}` 占位符，校验失败时由框架统一替换为 `$参数名`。例如 `#[Length(2, 20)] string $name` 校验失败时抛出：`$name 长度必须在 2 到 20 之间`
- **自定义消息**：同样支持 `{:name}` 占位符；未使用占位符时保持原样，框架不会自动附加参数名，避免污染开发者定稿的文案
- **类型错误**：容器参数注入的类型错误（如 `int` 收到 `null`）统一附加 `$参数名` 前缀，例如：`$age must be of type int, null given`

```php
class UserController
{
    public function create(
        // 自定义消息 + 占位符，失败时输出 "$name 长度必须在 2 到 20 之间"
        #[Length(2, 20, message: '{:name} 长度必须在 2 到 20 之间')] string $name,
        // 未使用占位符的自定义消息，失败时原样输出
        #[Mobile(message: '请输入正确的手机号')] string $phone,
    ) { }
}
```

> **注意**：规则实例由容器预建并在协程间共享复用，**禁止在规则实例上存储请求级状态**（如校验过程中的临时数据）。自定义规则需要参数名时，通过 `validate(mixed $value, string $name = '')` 的第二个参数接收，参见下文自定义校验规则。
...
use Viswoole\\Core\\Validate\\Rules\\{
    AlphaNumber, InArray, Mobile, Chinese, DateFormat, DateBefore
};
...
class ProfileController
{
    ...
    public function update(
        ...
        #[Mobile] string $phone,
        #[InArray(['male', 'female', 'other'])] string $gender, // 严格模式（===）为默认行为
        #[DateFormat('Y-m-d'), DateBefore('now')] string $birthday,
    ): array { /* ... */ }
}
```

> **说明**：`DateAfter` / `DateBefore` 的时间基准点支持 `Y-m-d H:i:s` 格式字符串、时间戳，以及 `+N` / `-N`（N 秒）相对当前时间的偏移语法。
...
class Phone extends BaseValidateRule
{
     ...
     * @param string $name - 参数名（可选），用于在错误消息中引用
...
class CustomEnum extends BaseValidateRule
{
    ...
    public function validate(mixed $value): mixed
    {
        if (!in_array($value, $this->allowed, true)) {
            ...
            $this->error("{:name} 必须在以下范围内: {$values}");
        }
        return $value;
    }
}
...
错误信息应明确指出问题和期望格式，并使用 `{:name}` 占位符引用参数名：

```php
// ✅ 推荐：错误信息具体有用，占位符自动替换为 $参数名
$this->error('{:name} 应为11位数字，以1开头');
...
规则实例由容器预建并在协程间共享复用，禁止在实例属性上存储请求级状态：

```php
// ✅ 推荐：需要上下文时通过 validate() 参数接收
public function validate(mixed $value, string $name = ''): mixed { }

// ❌ 不推荐：在实例上缓存请求级数据，会导致协程间状态泄漏
